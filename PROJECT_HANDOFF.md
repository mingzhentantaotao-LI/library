...
用户明确提出当前可能需要改变方向：先完成一个相对简单的网页，用于查询、上传、删除；数据库可以延后；文件管理 AI 化接入希望同步进行。

接手时应按新判断处理：

- 允许新增 `web/` 薄前端和轻量 API。
- 数据库仍延后，文件系统继续作为事实来源。
- 删除优先做成 `.trash/` 回收站式安全删除。
- AI 接入先做建议层和可插拔 hook，不直接自动改写稳定知识库。
- 对应决策记录见 `wiki/decisions/decision-002-thin-web-filesystem-first.md`。
